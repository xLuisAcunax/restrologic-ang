...
export class TableDetailsComponent implements OnInit, OnDestroy {
  ...
  private forceCreateNewOrder() {
    const branchId = this.getBranchId();
    if (!branchId) {
      this.error.set('No se encontrÃ³ la sucursal.');
      this.isSaving.set(false);
      return;
    }
  ...
  private syncOrderItems(orderId: string) {
    // Obtiene los items actuales, elimina los que ya no estÃ¡n y crea/actualiza los presentes en el carrito
    this.orderService.listOrderItems(orderId).subscribe({
      next: (serverItems) => {
        ...
        concat(...ops).subscribe({
          ...
          error: () => {
            this.error.set('No se pudieron sincronizar los Ã­tems.');
            this.isSaving.set(false);
          },
        });
      },
      error: () => {
        this.error.set('No se pudieron obtener los Ã­tems actuales.');
        this.isSaving.set(false);
      },
    });
  }
